...
enum { SchemeNorm, SchemeSel, SchemeHid, SchemeTagSel, SchemeTagNorm, SchemeTag, SchemeTag1, SchemeTag2, SchemeTag3, SchemeTag4, SchemeTag5, }; /* color schemes */
  ...
  int tw;
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* 2px right padding */
		else if (ev->x > selmon->ww - tw - getsystraywidth()+lrpad-2)
			click = ClkStatusText;
		else {
...
drawbar(Monitor *m)
{
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    w = TEXTW(tags[i]);
    drw_setscheme(drw, scheme[occ & 1 << i ? (m->colorfultag ? tagschemes[i] : SchemeTagSel) : SchemeTag]);
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (n > 0) {
			...
			drw_setscheme(drw, scheme[SchemeNorm]);
			// drw_setscheme(drw, scheme[SchemeNone]);
			drw_rect(drw, x, 0, w, bh, 1, 1);
		}
	}
	m->bt = n;
